...
\title{\textbf{论文实验说明（通俗易懂版）}\\[0.3em]
\large 尽量说人话 · 26 条实验逐条说明}
\author{对应英文论文实验部分 · 2026-07-10}
\date{}

\newcommand{\renhua}[1]{\par\smallskip\noindent\textbf{【人话一句话】}#1\par\smallskip}
...
\begin{abstract}
\noindent\textbf{这份文档是干什么的？}\\
帮你看懂英文论文里的\textbf{全部 26 个实验}：每个在测什么、怎么测、结果说明什么。按顺序读即可。

\noindent\textbf{怎么读？} 先读\ref{sec:plain_intro}导读 $\to$ \ref{sec:plaindict}词典 $\to$ \ref{sec:paper_experiments}全文解读 $\to$ 需要时再看各条实验的公式。

\noindent\textbf{两个方法：} \textbf{老 MACPO}（每轮必联网） vs \textbf{新方法}（先判断再联网，论文称 Proposed）。

\noindent\textbf{成绩 $F$：} 各组真实任务分之和，\textbf{越小越好}（电动车等场景越负越好）。罚分只是搜索辅助，不算进 $F$。
\end{abstract}
...
很多节点一起完成任务，但发消息很贵。老 MACPO 每轮都强制联网；新方法先看分歧大不大——小就不联，大或太久没联再联。实验要证明：联网少很多，分数不变差，往往更好。

\textbf{四个问题：}（1）分数会不会变差？（2）少联了多少网？（3）好结果靠「挑时机联网」还是「调参数」？（4）真实场景行不行？

\textbf{读表注意：} $F_1,\ldots,F_{18}$ 是 18 套题的名字；「配对实验」= 同一随机条件下老方法和新方法各跑一遍；「消融」= 关掉某功能看还灵不灵。

\section{见到词就查这里}\label{sec:plaindict}
\begin{center}\small
...
CI & 共享题差多少，越高越该联网 \\
eva & 一共算了多少次分 \\
胜/平/负 & 几道题新方法更好/差不多/更差 \\
保底联网 & 太久不联就强制联一次 \\
V0--V3 & 四种联网规则（V0=每轮必联） \\
\bottomrule
\end{longtable}
\end{center}
...
\subsection{问题}
多个节点一起优化，邻居之间有几道题必须答案一致。老 MACPO 每轮都强制联网对答案；新方法每轮先看值不值得联网。

\subsection{四个问题}
（1）分数会不会变差？（2）少联多少网？（3）好结果靠挑时机联网还是调参数？（4）真实场景行不行？

\subsection{一轮怎么跑}
本地搜答案 $\\to$ 算分歧 $\\to$ 决定联不联网 $\\to$ 重复直到改题次数用完。两种方法同一随机种子、同样改题上限，各 25 遍。
...
MACPO / 每轮必联 &
\textbf{老 MACPO（每轮必通信）} &
门控恒开 $g^t\equiv 1$，每外循环都协商；\emph{每轮必联} 即“通信常开”，不是算法正式名称。 \\
\addlinespace
新方法 / Full &
\textbf{新方法} &
...
\texttt{MACPO\_PAIR\_SEED} & paired seed & 环境变量：配对随机种子，保证 MACPO与新方法可比 \\
...
\addlinespace
w/t/l & win / tie / loss & 显著性检验下 新方法相对 MACPO 的胜/平/负函数个数 \\
$\Delta_k$ & paired improvement & 第 $k$ 次 run 上 MACPO与新方法的 $F$ 相对差 \\
FES$(e)$ & 目标函数值 vs evaluations & 累计 eva $\le e$ 时所见最优 $F$ \\
\bottomrule
\end{longtable}
\end{center}
...
\section{实验章节全文解读}\label{sec:paper_experiments}
\textbf{两种分数：} 搜索时 $h_i$ 带罚分；表格里的 $F$ 只加真实任务分。

\textbf{18 套题 F1--F18：} 三档规模（20/40/60 人）。F3、F9、F15 是同一类难题三份拷贝。

\textbf{主结论：} 新方法 18 题全胜；联网多数约 8\%；换调惩罚方式差不多，都远好于每轮必联——\textbf{新意在什么时候联网}。真实场景：发电几乎打平，调度/电动车大约好 10\%。

\section{实验 1：小档六道题：新方法 vs 老 MACPO}\label{sec:exp1}
%==============================================================================
\renhua{同样改题 25 遍，小档 6 题新方法全赢。}
\subsection{实验说明}
回答「问题 1」：小档 F1--F6，20 人链状相连。同样改题次数，比最终 $F$（越小越好）。比四组：老 MACPO、新方法、只调惩罚不挑时机、两种别家算法（参考）。25 遍配对，6 题全胜；只调惩罚在 F3/F5 明显不如完整版。

\subsection{实验计算公式}
\begin{itemize}
  \item 优化问题：式\eqref{eq:global}（Section~\ref{sec:notation}）；
  \item 门控逻辑：式\eqref{eq:local_gate}--\eqref{eq:gate}；\textbf{老 MACPO} 等价于恒令 $g^t\equiv 1$；
...
\begin{align}
  ...
  \quad\text{（胜/平/负：新方法相对 MACPO 在 0.05 水平下显著更优/无显著差异/显著更差的函数个数）} \label{eq:wtl}
\end{align}
配对 Wilcoxon 基于差值 $\Delta_k = F_k^{\mathrm{MACPO}} - F_k^{\mathrm{Proposed}}$（$\Delta_k>0$ 表示新方法更优）。
...
优化器 & LLSO（主）、CSO（次） \\
Runs & 25 配对独立运行 \\
新方法配置 & 完整版：门控（式\eqref{eq:local_gate}）+ PPO 调 $\alpha$ + 变量筛选 0.9/0.7/0.5 \\
\bottomrule
\end{tabular}
\end{center}
...
\end{tabular}
\end{center}
\textbf{结果说明}：新方法六函数全胜；\textbf{只调惩罚、不挑联网时机}在 F3/F5 弱于完整版，说明增益主要来自\textbf{冲突门控}，而非单靠 RL 调惩罚。

\tryfig{media/F1_F6_panel.pdf}{实验 1 配套图：F1--F6 单次配对实验 到目前最好收敛曲线}

%==============================================================================
\section{实验 2：中档大档十二题：新方法 vs 老 MACPO}\label{sec:exp2}
%==============================================================================
\renhua{12 题继续全赢；F9、F15 差距最大。}
\subsection{实验说明}
回答「问题 1」：F7--F18 放大版。报运行中出现过的最低 $F$。老 MACPO 在 F9、F15 对不齐共享题，分数差到离谱。12 题 25 遍，全赢。

\subsection{实验计算公式}
同 Section~\ref{sec:notation}；注意 MACPO 归档 最终有时记为带惩罚的 $F$，新方法主表报 $F$（真实任务分，式\eqref{eq:global} 右端不含 $\alpha c$）。
...
\subsection{实验设置与参数}
25 遍配对；数据源 MACPO\_original（MACPO）+ MACPO2\_deployment（RL）；LLSO/CSO 双优化器。
...
%==============================================================================
\section{实验 3：小档六题：成绩随改题次数变化（曲线）}\label{sec:exp3}
%==============================================================================
\renhua{曲线图：新方法分数降得更快或更低。}
\subsection{实验说明}
问题 1 补充图：每题挑一组配对记录，横轴改题次数，纵轴到此刻最低 $F$。一眼看出谁搜得更好。
...
%==============================================================================
\section{实验 4：跑一遍要多少秒}\label{sec:exp4}
%==============================================================================
\renhua{大约慢一倍；主结论看分数不看秒。}
\subsection{实验说明}
补充：单次运行秒数。新方法多算分歧和强化学习，F1 约 14s vs 27s。开发机参考，非主结论。
...
\subsection{算法伪代码}
同实验 1 运行流程；在相同 25 遍 批次上额外记录 \texttt{wall\_clock\_s}（硬编码于 tex，非 目标函数值 日志统一字段）。

\subsection{结果评估指标公式}
报告各函数 mean wall-clock（秒）；F1 LLSO 例：MACPO 14.43\,s，新方法 26.95\,s（约 1.9$\times$）。

\subsection{实验设置与参数}
开发机 参考性的测量；与 Table \texttt{tab:wall\_time\_f1\_f18} 一致。

\subsection{实验结果}
F1--F6 上 新方法约为 MACPO 的 1.7--2.1 倍；F7--F18 比值类似。通信削减带来的协商次数下降\textbf{未完全抵消} RL 模块开销，但 目标函数值 主指标仍全胜。

%==============================================================================
\section{实验 5：十八套题：联网占百分之几}\label{sec:exp5}
%==============================================================================
\renhua{多数题约 8\%，老方法 100\%。}
\subsection{实验说明}
回答「问题 2」：老方法 100\% 联网；新方法 F3--F6 约 8.3\%，F1 约 20.8\%。

\subsection{实验计算公式}
式\eqref{eq:pcomm}--\eqref{eq:comm}；\textbf{老 MACPO} 基线 $\bar p_{\mathrm{comm}} = 1$（100\%）。
...
\subsection{实验设置与参数}
LLSO，25 遍配对；\textbf{新方法}；脚本 \texttt{run\_comm\_rate\_f1\_f18.sh}。
...
%==============================================================================
\section{实验 6：小档六题：是不是少算题才少联网}\label{sec:exp6}
%==============================================================================
\renhua{不是，改题次数差不多。}
\subsection{实验说明}
回答「问题 2」：F1--F6 合并，新方法算分次数还略多 3.2\%。少联网不是因为少算题。
...
%==============================================================================
\section{实验 7：四种联网规则谁最好}\label{sec:exp7}
%==============================================================================
\renhua{要和最近平均水平比，且要有保底联网。}
\subsection{实验说明}
回答「问题 3」：V0 每轮必联；V1 固定线（30 遍全不联，失败）；V2 和平均水平比；V3 加保底联网。
...
%==============================================================================
\section{实验 8：联网灵敏度调大调小影响大吗}\label{sec:exp8}
%==============================================================================
\renhua{不会，默认 1.2 不是玄学数字。}
\subsection{实验说明}
问题 3：$\lambda$ 从 0.8 到 1.4，F3/F5 各 10 遍。分数没有剧烈变化。
...
%==============================================================================
\section{实验 9：固定每隔 K 轮必联}\label{sec:exp9}
%==============================================================================
\renhua{不如看分歧再联聪明。}
\subsection{实验说明}
问题 2 对照：每 2/3/5 轮强制联，不看分歧。联网比例相近时新方法分数仍更低。
...
\subsection{结果评估指标公式}
通信率 $= 1/K$（理想周期）；final $F$ 均值$\pm$标准差；与\textbf{新方法} 比较 Pareto 位置。

\subsection{实验设置与参数}
F1/F2/F5；PeriodicK2/K3/K5；10 遍试探（F125）或 25 遍（F1--F6 扩展）。

\subsection{实验结果}
\tryfig{media/comm_fitness_pareto_f125.pdf}{实验 9/12：通信率--适应度 Pareto 前沿（F1/F2/F5）}
\textbf{新方法}在相近通信预算下 目标函数值 更低，优于固定周期策略。

%==============================================================================
\section{实验 10：小档六题：换几种联网规则}\label{sec:exp10}
%==============================================================================
\renhua{无保底会联不上或结果飘。}
\subsection{实验说明}
问题 2：F1--F6 换几种联网规则。无保底可能 0\% 联网但结果飘。
...
\subsection{实验结果}
无 fail-safe 时通信率可为 0、目标函数值 方差极大；\textbf{完整版}在通信--质量权衡上最优。例：F3 完整版通信 8.3\%，FixedThresholdNoFailSafe 通信 0\% 但结果不稳定。

%==============================================================================
\section{实验 11：分歧大是否更容易联网}\label{sec:exp11}
%==============================================================================
\renhua{低分歧更常跳过——合理。}
\subsection{实验说明}
问题 3：按分歧分五档看联网概率。低分歧更常跳过。
...
\subsection{实验设置与参数}
F1--F6，25 遍，日志 \texttt{RL-output\_runs25\_rho/}；脚本 \texttt{plot\_ci\_bin\_trigger.py}。

\subsection{实验结果}
\tryfig{media/ci_bin_trigger_F1_F6.pdf}{F1--F6 CI 五等分位条件触发概率}

%==============================================================================
\section{实验 12：联网多少 vs 分数多少}\label{sec:exp12}
%==============================================================================
\renhua{左下角最好；新方法通常在那。}
\subsection{实验说明}
问题 2：横轴联网比例，纵轴 $F$；左下角最好。

\subsection{实验计算公式}
横轴 $\bar p_{\mathrm{comm}}$，纵轴 最终$F$；各方法为 $(\bar p, \overline{f})$ 散点。

\subsection{算法伪代码}
汇总\textbf{完整版}、PeriodicK2/3/5、老 MACPO 等方法的 $\bar p_{\mathrm{comm}}$ 与 $F$，绘制 Pareto 图。
...
\textbf{新方法（冲突门控）}占据更优前沿区域。

%==============================================================================
\section{实验 13：F3/F5 单条日志诊断}\label{sec:exp13}
%==============================================================================
\renhua{看冲突和惩罚怎么一起变。}
\subsection{实验说明}
问题 3 诊断：F3/F5 一条记录，画冲突、惩罚曲线。
...
%==============================================================================
\section{实验 14：和 MASOIE 比（参考）}\label{sec:exp14}
%==============================================================================
\renhua{路线不同，不能严格比；新方法仍强很多。}
\subsection{实验说明}
外部参考：MASOIE 规则不对齐，不能严格比。F3/F5 新方法仍好一个数量级。

\subsection{实验计算公式}
同式\eqref{eq:global}；MASOIE 使用作者可执行文件，无 MACPO 式惩罚协商。

\subsection{算法伪代码}
分别运行\textbf{新方法}（25 遍）与 MASOIE 官方流程（25 遍），比较 final $F$。
...
\midrule
新方法 & $\mathbf{6.35\times10^2 \pm 3.12\times10^1}$ & $\mathbf{3.50\times10^6 \pm 1.40\times10^5}$ \\
...
%==============================================================================
\section{实验 15：换三种调惩罚方式}\label{sec:exp15}
%==============================================================================
\renhua{差不多，都远好于每轮必联——关键在何时联网。}
\subsection{实验说明}
问题 3 核心：联网规则固定，换固定表/滑动平均/强化学习调惩罚。三种差不多，都远好于每轮必联。
...
\For{each \textit{controller} $\in \{\mathrm{MACPO}, \mathrm{Fixed}, \mathrm{EMA}, \mathrm{RL}\}$}
  ...
  \State Log 到目前最好$F$ and $\bar p_{\mathrm{comm}}$
\EndFor
...
\subsection{结果评估指标公式}
到目前最好$F$ + $\bar p_{\mathrm{comm}}$；gated 组间 Wilcoxon 无显著差异。
...
%==============================================================================
\section{实验 16：门控拆几层哪层关键}\label{sec:exp16}
%==============================================================================
\renhua{「和平均水平比」那层缺了会飘。}
\subsection{实验说明}
...
\subsection{算法伪代码}
在\textbf{完整版}栈上开关各层，25 遍 批量运行 F1--F6。
...
%==============================================================================
\section{实验 17：早中晚抽样、更新步长}\label{sec:exp17}
%==============================================================================
\renhua{主要改联网时机，不太改最终分。}
\subsection{实验说明}
问题 3：关或开早中晚抽样，换更新步长。最终分差别通常 $<5\%$。

\subsection{实验计算公式}
$\eta$ 为惩罚更新步长/比率；策略 Fixed / Phase / Eta\_RL / Unified 仅改变 $\alpha_i$ 更新规则，门控不变。

\subsection{算法伪代码}
固定 第1层+2+变量选择，切换 Phase 或 $\eta$ 模块，25 遍 $\times$ F1--F6。
...
%==============================================================================
\section{实验 18：换六种强化学习网络}\label{sec:exp18}
%==============================================================================
\renhua{分数差通常小于 5\%。}
\subsection{实验说明}
问题 3：换六种神经网络。差别 $<5\%$。
...
\subsection{算法伪代码}
固定门控与 eva 上限，仅替换 $\pi_\theta$ 网络与训练算法，25 遍 $\times$ F1--F6。
...
%==============================================================================
\section{实验 19：强化学习过程曲线}\label{sec:exp19}
%==============================================================================
\renhua{诊断用，不是主结论。}
\subsection{实验说明}
诊断：25 遍平均画惩罚、冲突、奖励曲线。不证明强化学习比查表强。
...
\subsection{实验结果}
\tryfig[0.48\textwidth]{media/rl_metrics_mean25_rho_alpha.pdf}{25 遍 均值：$\alpha$ 轨迹}
\tryfig[0.48\textwidth]{media/rl_metrics_mean25_rho_rho.pdf}{25 遍 均值：$\rho$ 轨迹}
\tryfig[0.48\textwidth]{media/rl_metrics_mean25_rho_conflict.pdf}{25 遍 均值：冲突代理轨迹}
\tryfig[0.48\textwidth]{media/rl_metrics_mean25_rho_reward.pdf}{25 遍 均值：RL reward 轨迹}

%==============================================================================
\section{实验 20：冲突和惩罚的关系}\label{sec:exp20}
%==============================================================================
\renhua{过程规律，诊断用。}
\subsection{实验说明}
诊断：按分歧分档看平均惩罚。
...
\subsection{算法伪代码}
脚本 \texttt{plot\_conflict\_alpha\_bins.py}；输入 25 遍 轨迹。
...
%==============================================================================
\section{实验 21：不联网时参考值漂不漂}\label{sec:exp21}
%==============================================================================
\renhua{漂移很小，保底联网合理。}
\subsection{实验说明}
诊断：不联网轮次参考值单步漂移约 0.004，很小。
...
\subsection{实验设置与参数}
F1--F6 headline \textbf{新方法}，25 runs/函数。
...
%==============================================================================
\section{实验 22：13 台机组发电}\label{sec:exp22}
%==============================================================================
\renhua{少联约 88\% 网，成本几乎不变。}
\subsection{实验说明}
问题 4：13 台机组，25 遍，联网约 11\%，成本差 0.05\%。
...
\end{equation}
$\Delta$：新方法相对 MACPO 的真实任务分改善百分比；$\Delta>0$ 表示新方法更优（$F$ 更低）。EV 场景符号约定见实验 24。
...
%==============================================================================
\section{实验 23：资源调度}\label{sec:exp23}
%==============================================================================
\renhua{少联约 88\% 网，收益大约好 10\%。}
\subsection{实验说明}
问题 4：10 节点资源调度，联网约 11.6\%，收益约好 11\%。
...
%==============================================================================
\section{实验 24：电动车充放电}\label{sec:exp24}
%==============================================================================
\renhua{这题越负越好；净收益大约好 10\%。}
\subsection{实验说明}
问题 4：10 节点电动车，$F$ 越负越好，净收益约好 10\%。
...
%==============================================================================
\section{实验 25：IEEE 30/57/118 电网}\label{sec:exp25}
%==============================================================================
\renhua{多数打平，118 节点略差 0.6\% 换少联网。}
\subsection{实验说明}
问题 4：IEEE 30/57/118，联网少 41--60\%，多数打平。
...
\begin{algorithmic}[1]
\For{$\textit{run\_id} = 1$ \textbf{to} $25$}
  \State Run MACPO and full 新方法 with fail-safe $K=2$
  \State Log 到目前最好$F$ and $\bar p_{\mathrm{comm}}$
...
\end{algorithm}
\varnote{
$\Delta_k$：第 $k$ 次 run 上 MACPO相对新方法 的成本变化百分比（见下式）。
...
%==============================================================================
\section{实验 26：参与者变多试探}\label{sec:exp26}
%==============================================================================
\renhua{只试 5 遍，不能当主结论。}
\subsection{实验说明}
试探：人变多，F1/F7/F13 只 5 遍，不能和 25 遍主表比。

\subsection{实验计算公式}
同 Section~\ref{sec:notation} 式\eqref{eq:global}--\eqref{eq:gate}；F1S50/S100 为 F1 的 50/100 智能体扩展实例。

\subsection{算法伪代码}
F1/F7/F13：5 遍试探；F1S50/S100：25 遍；MACPO vs 新方法 配对。

\subsection{结果评估指标公式}
$\bar p_{\mathrm{comm}}$ + final 目标函数值 均值$\pm$标准差。

\subsection{实验设置与参数}
5 遍（F1/F7/F13）或 25 遍（F1S50/S100）；\textbf{不可与 25 遍 主表直接混读}。
