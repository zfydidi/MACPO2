...
\subsection{单轮执行流程}
执行局部搜索 $\\to$ 计算冲突指标 $\\to$ 决定是否触发通信 $\\to$ 重复直到评估次数用完。两种方法同一随机种子、同样评估预算，各独立重复 25 次。
...
\texttt{MACPO\_PAIR\_SEED} & paired seed & 环境变量：配对随机种子，保证 MACPO 与本文方法可比 \\
...
$\Delta_k$ & paired improvement & 第 $k$ 次 run 上 MACPO 与本文方法的 $F$ 相对差 \\
...
\textbf{主要结论：} 本文方法 18 项测试全部优于 MACPO 基线；通信触发率约为 8\%；换调节惩罚方式基本一致，都远优于每轮均触发协商——\textbf{新意在什么时候通信}。真实场景：发电几乎持平，调度/电动车大约好 10\%。

\section{实验 1：小规模六项测试：本文方法 vs MACPO 基线}\label{sec:exp1}
%==============================================================================
【一句话概括】同等评估预算下各运行 25 次，小规模 6 项本文方法全面占优。}
\subsection{实验目的与设计}
对应核心问题 1：小规模测试 F1--F6，20 个智能体链式拓扑。同样评估次数，对对比最终目标值 $F$（越小越好）。对对比四组配置配置：MACPO 基线、本文方法、仅调节惩罚、不改变通信策略、两种其他算法（参考）。25 次配对实验，6 项测试全面占优；只调节惩罚在 F3/F5 明显不如完整版。
...
$F$：不含惩罚项的真实目标值；$\bar p_{\mathrm{comm}}$：通信触发率；\textit{eva\_count}：实际调用目标函数的次数。
}

\subsection{评价指标}
\begin{align}
  F &= \sum_i f_i(\mathbf{X}_i)
  \quad\text{（不含惩罚项的真实目标值 $F$：不含惩罚项，越小越好）} \label{eq:fpure}\\
...
\end{tabular}
\end{center}
\textbf{结果说明}：本文方法六函数全部优于 MACPO 基线；\textbf{只调节惩罚、不挑通信时机}在 F3/F5 弱于完整版，说明增益主要来自\textbf{冲突门控}，而非单靠 RL 调节惩罚。
...
%==============================================================================
【一句话概括】12 项测试继续全面占优；F9、F15 差距最大。}
\subsection{实验目的与设计}
对应核心问题 1：F7--F18（更大规模版本）。报运行中出现过的最低 $F$。MACPO 基线 在 F9、F15 共享变量未能有效对齐，导致目标值差距悬殊。12 项测试各独立重复 25 次，全面占优。

\subsection{相关公式}
同 Section~\ref{sec:notation}；注意 MACPO 归档 最终有时记为带惩罚的 $F$，本文方法主表报 $F$（不含惩罚项的真实目标值，式\eqref{eq:global} 右端不含 $\alpha c$）。
...
\subsection{实验结果与分析}
LLSO 与 CSO 均为 \textbf{12/0/0} 全部优于 MACPO 基线。典型：F9 MACPO $3.05\times10^{11}$ vs RL $1.01\times10^3$；F15 MACPO $1.24\times10^{12}$ vs RL $8.41\times10^2$。高冲突 Rosenbrock 相关实例差距最大。

%==============================================================================
\section{实验 3：小规模六项：收敛曲线（目标值 vs 评估次数）}\label{sec:exp3}
%==============================================================================
【一句话概括】曲线图：本文方法目标值收敛更快或更低。}
\subsection{实验目的与设计}
...
\end{equation}
其中 $e$ 为累计评估次数，$F$ 为不含惩罚项的真实目标值（见式\eqref{eq:fpure}）。
...
%==============================================================================
【一句话概括】耗时约增加一倍；主结论以目标值为准，不关注耗时。}
\subsection{实验目的与设计}
补充：单次运行耗时（秒）。本文方法多计算冲突指标和强化学习，F1 约 14s vs 27s。开发机参考，非主结论。
...
\subsection{算法流程}
同实验 1 运行流程；在相同 25 次 批次上额外记录 \texttt{wall\_clock\_s}（硬编码于 tex，非 目标值 日志统一字段）。

\subsection{评价指标}
报告各函数 mean wall-clock（秒）；F1 LLSO 例：MACPO 14.43\,s，本文方法 26.95\,s（约 1.9$\times$）。

\subsection{实验设置}
数据来自开发机的参考性测量；与 Table \texttt{tab:wall\_time\_f1\_f18} 一致。

\subsection{实验结果与分析}
F1--F6 上 本文方法约为 MACPO 的 1.7--2.1 倍；F7--F18 比值类似。通信削减带来的协商次数下降\textbf{未完全抵消} RL 模块开销，但 优化效果指标仍全部优于 MACPO 基线。

%==============================================================================
\section{实验 5：十八项测试通信触发率统计}\label{sec:exp5}
%==============================================================================
【一句话概括】多数题约 8\%，MACPO 基线 100\%。}
\subsection{实验目的与设计}
...
%==============================================================================
【一句话概括】需与近期平均水平比较，且要有 fail-safe 通信。}
\subsection{实验目的与设计}
...
%==============================================================================
\section{实验 8：通信灵敏度参数变化影响大吗}\label{sec:exp8}
%==============================================================================
【一句话概括】阈值 $\lambda$ 在 0.8--1.4 范围内对最终结果影响不大，默认值 1.2 不是敏感参数。}
\subsection{实验目的与设计}
...
%==============================================================================
【一句话概括】无 fail-safe会联不上或结果波动较大。}
\subsection{实验目的与设计}
问题 2：F1--F6 换几种通信规则。无 fail-safe可能 0\% 通信但结果波动较大。
...
%==============================================================================
【一句话概括】Pareto 图中越接近左下角越好；本文方法在低通信-低目标值区域占优。}
\subsection{实验目的与设计}
...
%==============================================================================
【一句话概括】基本一致，都远优于每轮均触发协商——关键在何时通信。}
\subsection{实验目的与设计}
问题 3 核心：通信规则固定，换固定表/滑动平均/强化学习调节惩罚。三种基本一致，都远优于每轮均触发协商。
...
%==============================================================================
【一句话概括】「和平均水平比」那层缺了会出现较大波动。}
\subsection{实验目的与设计}
问题 3：对门控的三个层级逐步叠加，观察每增加一层对结果的影响。缺少第二层（相对阈值）时，F1 的变异系数高达 17.6%，说明该层对稳定性至关重要。
...
%==============================================================================
【一句话概括】六种 RL 网络架构的最终目标值差异在 5% 以内，与实验 15 结论一致。}
\subsection{实验目的与设计}
问题 3：换六种神经网络。差别 $<5\%$。
...
\subsection{实验结果与分析}
\tryfig{media/fig_deeprl_comparison.pdf}{六种 RL 架构 F1--F6 最终目标值对比}
...
\end{equation}
$\Delta$：本文方法相对 MACPO 的不含惩罚项的真实目标值改善百分比；$\Delta>0$ 表示本文方法更优（$F$ 更低）。EV 场景符号约定见实验 24。
...
%==============================================================================
【一句话概括】该场景下目标值目标值越负表示效果越好；净收益大约好 10\%。}
\subsection{实验目的与设计}
...
%==============================================================================
【一句话概括】规模试点实验仅运行 5 次，统计功效有限，结论不宜直接推广。}
\subsection{实验目的与设计}
...
\subsection{评价指标}
$\bar p_{\mathrm{comm}}$ + final 目标值均值$\pm$标准差。
