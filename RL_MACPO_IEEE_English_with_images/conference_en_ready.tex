...
\begin{abstract}
MACPO-style network distributed optimization (NDO) opens communication on nearly every outer loop, so agents often negotiate when local conflict is too weak to justify the cost. We propose RL-MACPO, an adaptive communication architecture on the MACPO backbone that combines conflict-gated communication, variable selection in the negotiation branch, and adaptive penalty control via a lightweight RL policy. RL adjusts penalty strength only; it does not replace MACPO's evolutionary search or the rule-based gate. On F1--F18 (25 paired runs), dispatch simulators, and IEEE transmission-network pilots, RL-MACPO cuts outer-loop negotiation triggers to about 8--23\% while matching or improving final fitness at the same evaluation budget. Mechanism studies on F3/F5 show that gating and selection account for most of the gain: RL, EMA, and fixed penalty schedules are statistically tied once gating is fixed.
\end{abstract}
...
Network-based distributed optimization (NDO) models robotics~\cite{ref_dopt_robotics_2024}, smart grids~\cite{ref_dopt_smartgrid_2024}, wireless sensor networks~\cite{ref_dopt_wsn_2023}, and vehicular networks~\cite{ref_dopt_iov_2024}, where agents on a graph optimize through local information and neighbor interaction~\cite{ref_decentralized_review_2023}. A defining structure is the mixture of private variables and shared edge-coupled variables, as in traffic assignment~\cite{ref_ndo_traffic_2022}, power dispatch~\cite{ref_ndo_dispatch_2024}, distributed state estimation~\cite{ref_ndo_stateest_2024}, and related networked control~\cite{ref_ndo_netcontrol_2024}. When objectives are black-box and only locally observable, coordination proceeds without analytic gradients through repeated local search and neighbor negotiation on shared dimensions.

Although these methods have advanced the field, distributed black-box optimization still faces recurring limitations. Penalty and coordination parameters are often static or manually tuned, which limits adaptivity across conflict patterns and search stages. Communication and negotiation can dominate runtime in large or conflict-heterogeneous instances when negotiation is triggered on nearly every outer loop. The cost of negotiation also scales with both the number of shared variables processed and the frequency of communication, so always-on coordination becomes increasingly expensive as problem scale grows. Penalty-based cooperative evolution, exemplified by MACPO \cite{ref_macpo}, provides a practical NDO backbone, yet still triggers negotiation on nearly every outer loop and relies on static or manually tuned penalty control.
...
We ask how to reduce unnecessary communication in network-based distributed optimization while preserving optimization quality. MACPO opens communication and negotiates on nearly every outer loop; when conflict is weak or stale, this coordination cost may not translate into better shared-variable consistency or final fitness. Our response is an adaptive communication architecture (not a standalone RL penalty algorithm) with one design goal: communicate only when subsequent negotiation is worth the cost, and negotiate only on the shared dimensions that matter.

RL-MACPO realizes this architecture through three coordinated modules on the MACPO backbone. Conflict-gated communication decides whether to open the negotiation channel from online conflict signals, together with interval, relative-threshold, and fail-safe rules. Variable selection for negotiation decides which shared dimensions enter negotiation once communication is granted. Adaptive penalty control adjusts penalty strength from conflict dynamics through a common interface that supports RL and heuristic controllers; RL serves as a unified default controller without benchmark-specific schedule design. Fig.~\ref{fig:rl_macpo_architecture} summarizes the framework.

We present an adaptive communication architecture for MACPO-style distributed black-box optimization, combining conflict-gated communication, variable selection inside the negotiation branch, and adaptive penalty control as one coordinated design (Section~\ref{sec:method}, Algorithms~\ref{alg:ci_gating}--\ref{alg:negotiation_consistency} and~\ref{alg:rl_penalty_update}). We provide an adaptive penalty-control interface that supports RL and heuristic controllers under the same gating and selection architecture, with RL as a unified default that avoids benchmark-specific penalty schedules (Section~\ref{sec:penalty_interface}, Algorithm~\ref{alg:rl_penalty_update}). We validate the design on the MACPO NDO benchmark, through mechanism and ablation studies, and on MACPO-style dispatch simulators plus IEEE transmission-network pilots, organized around communication efficiency, controller choice, and module contributions (Section~\ref{sec:experiments}).
...
RL-MACPO is an adaptive communication architecture on the MACPO backbone (Section~\ref{sec:macpo_prelim}). Throughout the paper, communication means the gate decision (whether to open the channel) and negotiation means shared-variable processing after communication is granted. The default Full configuration enables conflict gating, variable selection, and adaptive penalty control with RL as the default implementation; ablations disable individual modules. Fig.~\ref{fig:rl_macpo_architecture} summarizes the per-iteration pipeline: local search and penalty application, conflict-gated communication, optional negotiation on selected shared variables, consensus update, and adaptive penalty control (RL default) before the next outer loop.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{media/fig2.pdf}
\caption{RL-MACPO per-iteration pipeline on the MACPO backbone. Steps~1--3 perform local optimization, penalty application, and conflict-gated communication; the negotiation branch (Steps~4--8) runs only when the gate opens; Step~9 adapts the penalty weight through the plug-in controller (RL default).}
\label{fig:rl_macpo_architecture}
\end{figure}

\paragraph{Pipeline reading guide.} Reading Fig.~\ref{fig:rl_macpo_architecture} from top to bottom, each agent first evolves its local population under the current penalty, then evaluates whether communication is warranted from the logged conflict proxy. If the gate opens, neighbors exchange and compete on a subset of shared dimensions; otherwise the iteration skips negotiation and only refreshes the consensus reference. After population re-evaluation, the penalty module updates $\alpha_i$ for the next outer loop through the shared interface in Section~\ref{sec:penalty_interface}.
...
\mathcal{C}_{\mathrm{rel}}^t &: \mathrm{CI}_i^t > \lambda\,\hat{\mu}_{\mathrm{CI},i}^t,\\
\mathcal{C}_{\mathrm{phase}}^t &: u_i^t < p_{\mathrm{phase}}^t,\quad u_i^t\sim\mathcal{U}(0,1),\\
\mathcal{C}_{\mathrm{fs}}^t &: t-t_{\mathrm{last\_comm}}\ge K.
\end{align}
When $\mathcal{C}_{\mathrm{fs}}^t$ holds, communication is forced and phase sampling is bypassed (matching the implementation). Here $\mathrm{CI}_i^t$ is a robust local conflict proxy computed from the difference between the local solution and the reference solution, $\hat{\mu}_{\mathrm{CI},i}^t$ characterizes the typical scale of the conflict signal in the current phase, $\lambda$ controls the relative threshold strength, $K$ caps the longest silent interval before fail-safe communication, and $p_{\mathrm{phase}}^t$ adjusts communication density across search stages.
...
\caption{Experimental roadmap (left to right). Q1--Q4 test optimization quality, communication reduction, mechanism contributions, and dispatch transfer, respectively; arrows indicate the recommended reading order through Sections~\protect\ref{sec:exp_q1}--\protect\ref{sec:applications}.}
...
MACPO's always-on negotiation repeatedly perturbs shared variables even when local conflict is weak. Conflict gating suppresses low-benefit rounds (Eq.~\eqref{eq:local_gate}), and variable selection limits negotiation to high-impact dimensions, shifting coordination from quantity to quality. On F1--F18, communication becomes much sparser while fitness improves at the same budget (Tables~\ref{tab:macpo_style_all},~\ref{tab:comm_rate_f1_f18}). Once gating and selection are fixed, penalty-controller choice is not dominant: Table~\ref{tab:penalty_controller_f3_f5} shows several adaptive implementations are statistically tied, so the architecture rather than a particular penalty schedule accounts for most of the gain.

We retain RL as the default penalty implementation because it provides a unified adaptive controller without benchmark-specific schedule design and matches EMA and fixed schedules on F3/F5 without significant Wilcoxon differences. Fixed schedules require hand-designed phase or interval rules; EMA requires smoothing parameters and update heuristics. RL learns the adjustment policy from optimization feedback (Eq.~\eqref{eq:rl_reward}) through the shared penalty interface. That is an automation advantage, not a claim of universal superiority.

Comparisons are mainly against MACPO and internal variants rather than a broad set of external distributed black-box optimizers. Gate validation (Table~\ref{tab:gate_cost_stats}) uses a 30-run F1/F2/F5 subset. IEEE~118 shows a small detectable cost offset alongside much lower communication. IEEE~30 has two seed-level stagnation outliers unrelated to communication rate. We do not provide convergence guarantees for the RL-augmented loop. Asynchronous graphs and multi-hop negotiation remain open extensions.

\section{Conclusion}\label{sec:conclusion}
RL-MACPO is an adaptive communication architecture for network-based distributed black-box optimization on the MACPO backbone. Conflict gating, variable selection, and a plug-in penalty interface work together so that communication opens on only a small fraction of outer loops. On F1--F18, final fitness improves at the same evaluation budget; dispatch pilots show similar communication savings with preserved objective quality. Controller comparisons on F3/F5 confirm that architecture, not penalty choice alone, drives the gain. RL remains the default penalty controller because it avoids benchmark-specific schedules, not because it dominates every comparison.
