...
from utils.power_case_table import build_ieee_power_table_tex, load_ieee_power_data
...
def _rl_objective_better(key: str, macpo_val: float, rl_val: float, *, min_rel: float = 0.005) -> bool:
    """Bold RL only when improvement is at least min_rel (default 0.5%)."""
    if rl_val >= macpo_val:
        return False
    denom = max(abs(macpo_val), 1e-12)
    return (macpo_val - rl_val) / denom >= min_rel
...
def build_application_section_tex(data: dict[str, Any] | None = None) -> str:
    ...
    return "\n".join(
        [
            ...
            r"(\texttt{run\_maed.sh}, \texttt{run\_paper\_scenarios.sh}, \texttt{run\_power.sh}); "
            r"aggregated summaries match \texttt{patent\_supplement/paired\_experiment\_data.json} "
            r"and \texttt{power\_dispatch\_sim/output/}.",
            r"",
            table,
        ]
    )
...
def build_ieee_power_section_tex(data: dict[str, Any] | None = None) -> str:
    data = data or load_ieee_power_data()
    if not data:
        return ""
    table = build_ieee_power_table_tex(data)
    ieee57 = data.get("IEEE57", {})
    ieee118 = data.get("IEEE118", {})
    parts = [
        r"\subsubsection{IEEE Transmission-Network Dispatch}",
        r"We further evaluate paired MACPO vs.\ RL-MACPO on standard IEEE 30-, 57-, and "
        r"118-bus regional dispatch instances generated by \texttt{power\_dispatch\_sim/} "
        r"(10 paired runs per case, identical evaluation caps). "
        r"Table~\ref{tab:ieee_power_cases} reports communication reduction together with "
        r"best pure dispatch cost. On the 118-bus case, RL-MACPO lowers communication by "
        f"about {float(ieee118.get('comm_reduction_pct', 0)):.0f}\\% while best cost changes "
        f"by only {abs(float(ieee118.get('best_f_pure_improvement_pct', 0))):.2f}\\%; on the "
        r"57-bus case, communication drops by "
        f"about {float(ieee57.get('comm_reduction_pct', 0)):.0f}\\% with a best-cost change of "
        f"{float(ieee57.get('best_f_pure_improvement_pct', 0)):+.1f}\\%. "
        r"The 30-bus instance reaches a shared cost plateau in the current configuration "
        r"(near-tied best $f_{\mathrm{pure}}$), but still shows reduced negotiation frequency.",
        r"",
        table,
    ]
    return "\n".join(parts)
...
def build_full_application_section_tex(data: dict[str, Any] | None = None) -> str:
    base = build_application_section_tex(data)
    ieee = build_ieee_power_section_tex()
    if ieee:
        return base + "\n\n" + ieee
    return base
