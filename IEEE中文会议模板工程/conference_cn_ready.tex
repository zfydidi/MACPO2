...
为直观说明惩罚项在该类网络型分布式优化问题中的作用，图~\ref{fig:penalty_landscape} 给出了代表性测试函数在有惩罚项与无惩罚项条件下的对比示意。该图的作用不是比较 RL-MACPO 与其他方法的最终性能，而是说明：当共享变量一致性约束被显式纳入目标后，局部搜索所面对的优化景观会发生变化，因此惩罚机制本身就是 MACPO 及其改进框架中的关键组成部分。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{media/img.pdf}
\caption{Illustration of representative benchmark functions with and without penalty terms. This figure is used to explain the role of penalty in shaping the optimization landscape, rather than to compare final algorithm performance.}
\label{fig:penalty_landscape}
\end{figure}
...
\begin{tikzpicture}[
  font=\small,
  rlbox/.style={draw=black, rounded corners=2pt, line width=0.8pt, minimum width=4.0cm, minimum height=1.15cm, align=center, fill=white},
  rlsmall/.style={draw=black, rounded corners=2pt, line width=0.8pt, minimum width=3.3cm, minimum height=1.10cm, align=center, fill=white},
  rlarrow/.style={-Latex, line width=0.9pt, draw=black}
]

\node[rlbox] (state) at (0,0) {State Input\\(conflict strength, conflict trend)};
\node[rlbox] (policy) at (6.3,0) {Policy Network\\(SimpleNet, $\tanh$)};
\node[rlsmall] (action) at (12.0,0) {Action Output\\$a_i^{(t)}$};
\node[rlsmall] (weight) at (12.0,-3.0) {Weight Update\\$w_i^{(t+1)}$};
\node[rlsmall] (reward) at (6.3,-3.0) {Reward Feedback\\$r_i^{(t)}$};

  \draw[rlarrow] (state) -- (policy);
  \draw[rlarrow] (policy) -- (action);
  \draw[rlarrow] (action) -- (weight);
  \draw[rlarrow] (weight) -- (reward);
  \coordinate (feedbackTurn) at ($(state.south)+(0,-1.35)$);
  \draw[rlarrow] (reward.west) -| (feedbackTurn) -| (state.south);
...
\begin{tikzpicture}[
  font=\small,
  selbox/.style={draw=black, rounded corners=2pt, line width=0.9pt, minimum width=4.8cm, minimum height=1.0cm, align=center, fill=white},
  seldiamond/.style={diamond, draw=black, line width=0.9pt, aspect=2.3, inner sep=2.2pt, align=center, fill=white},
  selarrow/.style={-Latex, line width=1.0pt, draw=black}
]

% top vertical chain
\node[selbox] (conflict) at (0,6.2) {Conflict Contribution Estimation};
\node[selbox] (score)    at (0,4.7) {Update Variable Importance Score};
\node[selbox] (rank)     at (0,3.2) {Rank Shared Variables};
\node[seldiamond, minimum width=5.8cm, minimum height=1.55cm] (cand) at (0,1.35) {Selected Candidate Set?};

% left and right branches from first decision
\node[selbox, minimum width=4.0cm] (skip) at (-6.1,1.35) {Skip This Variable};
\node[selbox, minimum width=4.2cm] (vars) at (6.1,1.35) {Candidate Variables};

% second decision and final outputs
\node[seldiamond, minimum width=6.4cm, minimum height=1.75cm] (check) at (6.1,-1.25) {Consistency Check\\and Trial Replacement};
\node[selbox, minimum width=4.3cm] (off) at (2.5,-3.9) {$\mathrm{variable\_switch}[d]=0$};
\node[selbox, minimum width=4.3cm] (on)  at (9.7,-3.9) {$\mathrm{variable\_switch}[d]=1$};

% main arrows
\draw[selarrow] (conflict) -- (score);
\draw[selarrow] (score) -- (rank);
\draw[selarrow] (rank) -- (cand);
\draw[selarrow] (cand) -- node[above, font=\scriptsize] {N} (skip);
\draw[selarrow] (cand) -- node[above, font=\scriptsize] {Y} (vars);
\draw[selarrow] (vars) -- (check);
\draw[selarrow] (check) -- node[above left, font=\scriptsize] {N} (off);
\draw[selarrow] (check) -- node[above right, font=\scriptsize] {Y} (on);

\end{tikzpicture}%
}
\caption{Smart variable selection and switch control.}
...
图~\ref{fig:main_results_bar} 以对数尺度展示了三种方法在 F1--F6 上的最终适应度，误差线对应 25 次运行的标准差。图~\ref{fig:main_comparison} 给出了 MACPO 与 RL-MACPO 的收敛曲线，其中横轴为评估次数，纵轴为对数尺度下的适应度。图~\ref{fig:fes_comparison} 则进一步从函数评估次数（FES）的角度展示了不同方法在代表性测试函数上的动态优化过程，用于观察优化早期、中期和后期的行为差异。整体来看，RL-MACPO 在多数函数上收敛更快，并达到更低的最终适应度。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{media/FES.pdf}
\caption{FES-based convergence comparison of MACPO, RL Only, and RL-MACPO on representative benchmark functions. The horizontal axis is the number of function evaluations, and the vertical axis is the fitness value in log scale.}
\label{fig:fes_comparison}
\end{figure}
...
\begin{table}[htbp]
  ...
  \toprule
  \textbf{Method} & \textbf{Zero-comm Runs} & \textbf{Comm Trigger Rate} & \textbf{Avg. Nego Dims} & \textbf{RL Updates / Run} & \textbf{Avg. Wall Time (s)} & \textbf{Comm Reduction} \\
  \midrule
